                    ...
                    for (Element good : elements) {
                        ...
                        if (!ifContain) {
                            ...
                            if (priceElement.selectFirst("i") != null) {
                                goodsPrice = NumberUtils.toDouble(priceElement.selectFirst("i").text());
                            }
                            String sql = "insert into tb_spider (goods_name,url,goods_price) values(?,?,?) ";
                            int update = jdbcTemplate.update(sql, goodsName, url, goodsPrice);
//                            int update = goodsInfoService.addGoodsInfo(goodsName, goodsPrice, url);
                            if (update > 0) {
